...
const createUser = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db().collection("cse341_spring_2025").insertOne(user);
...
const updateUser = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db().collection("cse341_spring_2025").replaceOne({_id: userId}, user);
...
const deleteUser = async (req, res) => {
    //#swagger.tags=['Users']
    const userId = new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db().collection('cse341_spring_2025').deleteOne({_id:userId});
